...
$jpegs = Get-ChildItem $root -Recurse -Include *.jpg,*.jpeg | Sort-Object FullName

$ok = 0; $fail = 0; $savedBytes = 0L
foreach ($jpg in $jpegs) {
    $webp = [System.IO.Path]::ChangeExtension($jpg.FullName, ".webp")
    $rel = $jpg.FullName.Substring($root.Length + 1)
    Write-Host ("-> {0} ({1:N2} MB)" -f $rel, ($jpg.Length/1MB))
    if (Test-Path $webp) { Remove-Item $webp -Force }
    & $ffmpeg -y -loglevel error -i $jpg.FullName -c:v libwebp -quality 70 $webp
    if ($LASTEXITCODE -eq 0 -and (Test-Path $webp)) {
        $newSize = (Get-Item $webp).Length
        $delta = $jpg.Length - $newSize
        $savedBytes += $delta
